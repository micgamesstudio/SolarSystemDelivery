...
import { createNoise3D } from './simplex-noise/simplex-noise.js';
...
/*PIANETA SIMPLEX*/
function E3_GenerateSimplexPlanetGeom(Obj) {
   /*
   const PlanetGeom = E3_GenerateSimplexPlanetGeom({
      Seed: 'seed',       //SEME DEL NOISE
      Diametro: 5,        //DIAMETRO DELLA SFERA
      Picco: 0.2,         //PICCO MASSIMO
      Frequenza: 5,       //FREQUENZA DEGLI AVVALLAMENTI
   });
   */

   const random = new Alea(Obj.Seed);
   const noise3D = createNoise3D(random);

   const geometry = new THREE.IcosahedronGeometry(100, 50);
   ...
   for (let i = 0; i < pos.count; i++) {
      ...
      vertex.normalize();
      const n = noise3D(
         vertex.x * Obj.Frequenza,
         vertex.y * Obj.Frequenza,
         vertex.z * Obj.Frequenza
      );
      const height = Obj.Diametro + n * Obj.Picco;     //10 + n * 1.5
...
async function E3_MaterialeLucido(Object) {
   /*
   const MaterialeLucido = E3_MaterialeLucido({
      Color: 0xd1ffff,
      RepeatX: 1,
      RepeatY: 1,
      FlatShading: true,
      Shininess: 120,
      Specular: Colors.SpecularColor,     //RIFLESSO LUCE DEL SOLE
      Side: "Double",          //"Front", "Double"
      Transparent: true,
      DepthWrite: false,             //SI COMPORTA DA MATERIALE SOLIDO OPACO (DEFAULT)
...
async function E0_Editor() {
   ...
   const GeometrySubFolders = [];
   GeometryFolder.close();
   GeometrySubFolders[0] = GeometryFolder.addFolder("E3_GenerateFilamentCloud");
   GeometrySubFolders[0].close();
   GeometrySubFolders[1] = GeometryFolder.addFolder("E3_GenerateSimplexPlanetGeom");
   GeometrySubFolders[1].close();
   ...
   GeometrySubFolders[0].add(settings, `E3_GenerateFilamentCloud`);
   GeometrySubFolders[0].add(settings.E3_GenerateFilamentCloud_Param, "shape", ["cube", "sphere"]).name("shape");
   GeometrySubFolders[0].add(settings.E3_GenerateFilamentCloud_Param, "count", 1000, 20000).step(100).name("count");
   GeometrySubFolders[0].add(settings.E3_GenerateFilamentCloud_Param, "spaceSize", 10, 100).step(10).name("spaceSize");
   GeometrySubFolders[0].add(settings.E3_GenerateFilamentCloud_Param, "numFilaments", 1, 100).step(1).name("numFilaments");
   GeometrySubFolders[0].add(settings.E3_GenerateFilamentCloud_Param, "filamentLength", 1, 50).step(1).name("filamentLength");
   GeometrySubFolders[0].add(settings.E3_GenerateFilamentCloud_Param, "filamentSegments", 1, 100).step(1).name("filamentSegments");
   GeometrySubFolders[0].add(settings.E3_GenerateFilamentCloud_Param, "filamentRadius", 1, 20).step(1).name("filamentRadius");
   GeometrySubFolders[0].add(settings.E3_GenerateFilamentCloud_Param, "filamentDensity", 0, 1).step(0.1).name("filamentDensity");
   //#endregion

   //E3_GenerateSimplexPlanetGeom
   //#region
   settings[`E3_GenerateSimplexPlanetGeom_Param`] = {
      Diametro: 5,        //DIAMETRO DELLA SFERA
      Picco: 0.2,         //PICCO MASSIMO
      Frequenza: 5,       //FREQUENZA DEGLI AVVALLAMENTI
   };

   settings[`E3_GenerateSimplexPlanetGeom`] = function () {
      // const geometry = E3_GenerateFilamentCloud({
      //    shape: settings.E3_GenerateFilamentCloud_Param.shape,
      //    count: settings.E3_GenerateFilamentCloud_Param.count,
      //    spaceSize: settings.E3_GenerateFilamentCloud_Param.spaceSize,
      ...
      //    filamentDensity: settings.E3_GenerateFilamentCloud_Param.filamentDensity
      // });
      const geometry = E3_GenerateSimplexPlanetGeom({
         Seed: 'seed',       //SEME DEL NOISE
         Diametro: settings.E3_GenerateSimplexPlanetGeom_Param.Diametro,
         Picco: settings.E3_GenerateSimplexPlanetGeom_Param.Picco,
         Frequenza: settings.E3_GenerateSimplexPlanetGeom_Param.Frequenza,
      });
      const material = new THREE.PointsMaterial({ color: 0xffffff, size: 0.1 });
      const points = new THREE.Points(geometry, material);
      ImportedObject.clear();
...
/*----------------------------------------------1 CREATION ENGINE 2775 (12692-10904=1788)------------------------------------------------------*/
async function E0_CreationEngine(Obj, Oggetti, Geometrie, Materiali, Par, manager) {
   ...
   function ForEachGenerateMesh(Node, MeshSubGroup, i, x, y, Funzione) {
      let MeshVisible;
      if (Node.MeshArray[i].Visible == true) MeshVisible = true;
      if (Node.MeshArray[i].Visible == false) MeshVisible = false;
      if (!Node.MeshArray[i].Visible) MeshVisible = true;

      let X;
      if (Number.isFinite(x)) X = x + 1;
      else x = "";

      //SE NON È UN GRUPPO GENERA LA MESH
      if (Node.MeshArray[i].Group == false) {
         if (Node.MeshArray[i].Option == true) {
            const MeshGeometry = GenerateGeometry(Node.MeshArray[i]);

            //SCOPRIRE COSA FA E SE ELIMINARLO O IMPLEMENTARLO PER TUTTI
            ...
            if (typeof Node.MeshArray[i].Material === 'number') {
               E3_UniversalMesh({
                  //PARAMETRI OBBLIGATORI:
                  Geom: MeshGeometry,
                  Material: MaterialArray[Node.MeshArray[i].Material],
                  //PARAMETRI OPZIONALI
                  Type: Node.MeshArray[i].MeshType,
                  Name: Node.MeshArray[i].Name,
                  Position: Node.MeshArray[i].Position,
                  Rotation: Node.MeshArray[i].Rotation,
                  Scale: Node.MeshArray[i].Scale,
                  Visible: MeshVisible,
                  Shadows: Node.MeshArray[i].Shadows,
                  Group: MeshSubGroup
               });
            }
            else if (Node.MeshArray[i].Material == "StampDock") {
               const Stamp = E3_StampCanvas(Node.MeshArray[i].StampDock, X, y + 1);
               E3_UniversalMesh({
                  //PARAMETRI OBBLIGATORI:
                  Geom: MeshGeometry,
                  Material: Stamp,
         ...
         if (Node.MeshArray[i].Option == "Number") {
            if ((Node.MeshArray[i].Number.length == 2 && Node.MeshArray[i].Number[0] == x && Node.MeshArray[i].Number[1] == y) || (Node.MeshArray[i].Number.length == 1 && Node.MeshArray[i].Number[0] == y)) {
               const MeshGeometry = GenerateGeometry(Node.MeshArray[i]);
      ...
      if (Node.MeshArray[i].Group == true) {
         if (Node.MeshArray[i].Option == true) {
            MeshSubGroup.add(Funzione());
         };
         //SE È OPZIONABILE COME NUMERO
         if (Node.MeshArray[i].Option == "Number") {
            if ((Node.MeshArray[i].Number.length == 2 && Node.MeshArray[i].Number[0] == x && Node.MeshArray[i].Number[1] == y) || (Node.MeshArray[i].Number.length == 1 && Node.MeshArray[i].Number[0] == y)) MeshSubGroup.add(Funzione());
         };
      };
   };
   ...
   async function Generic2(Dir, Num, Name, GeomModel, VariabiliObj, Tractor, UniversalGeom) {
      ...
      if (UniversalGeom == null || UniversalGeom == false) {
         ...
         if (Array.isArray(Geometrie[GeomModel].Multi) && Geometrie[GeomModel].Multi.length > 0) {
            ...
            for (let a = 0; a < Geometrie[GeomModel].Multi.length; a++) {
               const Node = FindObjectValue(["Multi", GeomModel, a]);
               //CREA UN ARRAY DI GEOMETRIE PER OGNI MATERIALE
               const GeomArray = [];

               /*---------------------------------LIVELLO B - PER OGNI GEOMETRIA O GRUPPO DI GEOMETRIE------------------------------------*/
               for (let b = 0; b < Node.Geometry.length; b++) {
                  //SE NON È UN GRUPPO GENERA LA GEOMETRIA
                  if (Node.Geometry[b].Group == false) {
                     if (Node.Geometry[b].Option == true) {
                        GeomArray.push(GenerateGeometry(Node.Geometry[b]));
                     };
                  };

                  //SE È UN GRUPPO
                  if (Node.Geometry[b].Group == true) {
                     if (Node.Geometry[b].Option == true) {
      ...
      if (Array.isArray(Geometrie[GeomModel].Generic) && Geometrie[GeomModel].Generic.length > 0) {
         ...
         for (let a = 0; a < Geometrie[GeomModel].Generic.length; a++) {
            const Node = FindObjectValue(["Geometry", GeomModel, a]);
            //CREA UN ARRAY DI GEOMETRIE PER OGNI MATERIALE
            const GeomArray = [];
            ...
            for (let b = 0; b < Node.Geometry.length; b++) {
               //SE NON È UN GRUPPO GENERA LA GEOMETRIA
               if (Node.Geometry[b].Group == false && Node.Geometry[b].Option == true) GeomArray.push(GenerateGeometry(Node.Geometry[b]));

               //SE È UN GRUPPO - GENERA IL GRUPPO DI LIVELLO B E LE GEOMETRIE DI LIVELLO C
               if (Node.Geometry[b].Group == true && Node.Geometry[b].Option == true) GeomArray.push(GenerateGroupLevelB(a, b));
            };
      ...
      function GenerateMeshMultiLevelA(a) {
         ...
         for (let x = 0; x < GroupParam.Number; x++) {
            ...
            for (let b = 0; b < Node.MeshArray.length; b++) {
               ForEachGenerateMesh(Node, MeshSubGroup, b, 0, x, () => GenerateMeshMultiLevelB(a, b, x))
            };
      ...
      function GenerateMeshMultiLevelB(a, b, x) {
         ...
         for (let y = 0; y < GroupParam.Number; y++) {
            ...
            for (let c = 0; c < Node.MeshArray.length; c++) {
               ForEachGenerateMesh(Node, MeshSubGroup, c, x, y, null);
            };
      ...
      function GenerateMeshGroupLevelA(a) {
         ...
         for (let x = 0; x < GroupParam.Number; x++) {
            ...
            for (let b = 0; b < Node.MeshArray.length; b++) {
               ForEachGenerateMesh(Node, MeshSubGroup, b, "", x, () => GenerateMeshGroupLevelB(a, b, x))
            };
   ...
   /*----------------------------------------------FUNZIONI COMUNI PROMISES--------------------------------------------------------*/
   /*TROVA VALORE NELL'OGGETTO DI CONFIGURAZIONE*/
   function FindOggettiValue(Array) {
      /*
      const Node = FindOggettiValue(["PlanetarySystem", i, a, b]);
      */
      /*----------------------PROMISES PLANETARY SYSTEM (STAZIONI SPAZIALI)----------------------*/
      if (Array[0] == "PlanetarySystem") {
         if (Array.length == 3) return Oggetti.PlanetarySystem.Modular[Array[1]].Modular[Array[2]];
         if (Array.length == 4) return Oggetti.PlanetarySystem.Modular[Array[1]].Modular[Array[2]].Modular[Array[3]];
      };
   };
   ...
   if (Par.CreateObj.PlanetarySystem == true) {
      ...
      for (let i = 0; i < Oggetti.PlanetarySystem.Modular.length; i++) {
         //PER OGNI SUA LUNA
         for (let a = 0; a < Oggetti.PlanetarySystem.Modular[i].Modular.length; a++) {
            const Node = FindOggettiValue(["PlanetarySystem", i, a]);
            //GENERAZIONE OGGETTO GENERICO
            if (Node.GenericModel == true) {
               Promises[PromiseCount] = () => Generic2(
                  'PlanetarySystem',
                  Node.Model,          //Numero
                  Node.Name[0],        //Name
                  Node.GeomModel,           //NUMERO MODELLO NELL'ARRAY "GEOMETRIE"
                  Node.Variabili,      //OGGETTO VARIABILI
                  Node.Tractor.Enable,
                  Node.UniversalGeom,        //ARRAYGEOM
               );
               if (Node.Name[0]) Promises[PromiseCount]._name = Node.Name[0];
            ...
            // for (let b = 0; b < Oggetti.PlanetarySystem.Modular[i].Modular[a].Modular.length; b++) {
            for (let b = 0; b < Node.Modular.length; b++) {
               const SubNode = FindOggettiValue(["PlanetarySystem", i, a, b]);
               //GENERAZIONE OGGETTO GENERICO
               if (SubNode.GenericModel == true) {
                  Promises[PromiseCount] = () => Generic2(
                     'PlanetarySystem',
                     SubNode.Model,          //Numero
                     SubNode.Name[0],        //Name
                     SubNode.GeomModel,           //NUMERO MODELLO NELL'ARRAY "GEOMETRIE"
                     SubNode.Variabili,      //OGGETTO VARIABILI
                     SubNode.Tractor.Enable,
                     SubNode.UniversalGeom,        //ARRAYGEOM
                  );
                  if (SubNode.Name[0]) Promises[PromiseCount]._name = SubNode.Name[0];
...
export async function MicEngine(Parameters, OggettiObj, GeometrieObj, MaterialiObj) {
   ...
   MicEnginereturn.Geometries = {
      ...
      E3_GenerateSimplexPlanetGeom: E3_GenerateSimplexPlanetGeom,
   };
...
export async function SnippetEngine() {
   ...
   /*--------------------------------------------GENERAZIONE HUD-----------------------------------------------------*/
   //#region
   const SnippetHUDObj = {                     //S0_GenerateHUDCanvas
      Parent: "Document",
      Events: true,           //ABILITAZIONE DEGLI EVENTI AL CLICK
      DispatchEvent: "Render",    //"Render", null
      Width: 1,                   //LARGHEZZA
      ...
      PulsName: ["Home", "+", "-"],
      PulsSize: [
         { Width: "200px", Height: "40px" },     //HOME
         { Width: "90px", Height: "40px" },    //1 ZOOM +
         { Width: "90px", Height: "40px" },    //2 ZOOM -
      ],
      PulsPos: [
         { RightFlag: "Left", PosX: "110px", TopFlag: "Top", PosY: "30px" },     //HOME
         { RightFlag: "Right", PosX: "55px", TopFlag: "Top", PosY: "30px" },         //1 ZOOM +
         { RightFlag: "Right", PosX: "55px", TopFlag: "Bottom", PosY: "30px" },      //2 ZOOM -
      ],
      PulsColor: [
         "rgba(28, 61, 60, 1)",     //HOME
         "rgba(28, 61, 60, 1)",     //1 ZOOM +
         "rgba(28, 61, 60, 1)",     //2 ZOOM -
      ],
      //BARRE
      Barre: 0,
   };

   const SnippetHUDCanvas = S0_GenerateHUDCanvas(SnippetHUDObj);
   //PULSANTE HOME
   SnippetHUDCanvas.setButtonCallback(0, () => {
   ...
   //PULSANTE ZOOM +
   SnippetHUDCanvas.setButtonCallback(1, () => {
   ...
   //PULSANTE ZOOM -
   SnippetHUDCanvas.setButtonCallback(2, () => {
      MicEnginereturn.OrbitControl.setDollyTarget(MicEnginereturn.OrbitControl.getDistance() + MicEnginereturn.OrbitControl.getDistance() * 0.3);
   });

   SnippetHUDCanvas.render();
   //#endregion
   ...
   const SeaGeom = E3_GeoSphere(5, 64, 32, 0, Math.PI * 2, 0, Math.PI);
   const SeaMat = await E3_MaterialeLucido({
      Color: 0x00ffff,
      RepeatX: 1,
      RepeatY: 1,
      FlatShading: false,
      Shininess: 30,
      Specular: 0xffffff,     //RIFLESSO LUCE DEL SOLE
      Side: "Front",          //"Front", "Double"
      Transparent: true,
      DepthWrite: true,             //SI COMPORTA DA MATERIALE SOLIDO OPACO (DEFAULT TRUE)
      AlphaTest: 0,       //Abilitarlo al posto di "Transparent" quando si ha una texture con buchi netti (anelli planetari) (0-1)
