...
//VERSIONE COMPLETA CON POKI E CRAZYGAMES AUTOMATICI
...
async function E2_GeneratePlanet(Name, PlanetGeom, RingGeom) {
   ...
   const PlanetMaterial = await E3_MaterialeStandard({
      ...
      // MapLod: true,    //GENERA LE MIPMAP IN CASO DI TEXTURE NON KTX2
...
async function E0_DynamicPlanetMap() {
   ...
   async function UpdateTexture() {
      //SOLE
      // console.log(PlanSystem.children[0].children[0].material.map);
      ...
      for (let i = 0; i < Oggetti.PlanetarySystem.Modular.length; i++) {
         ...
         // console.log(PlanSystem.children[i + 1].children[0].children[0]);
...
/*----------------------------------------------1 CREATION ENGINE------------------------------------------------------*/
//PROVARE A FARE UN PO DI REFACTORING (10924 13706 = 2782)
async function E0_CreationEngine(Obj, Oggetti, Geometrie, Materiali, Par, manager) {
   ...
   function MoltiplicationGroup(Node) {
      ...
      //Generic2 - GenerateGroupLevelB, GenerateGroupLevelC, GenerateGroupLevelD
      //GenericGeometry - GenerateMultiGroupLevelB, GenerateMultiGroupLevelC, GenerateMultiGroupLevelD
      //GenericRecycledGeometry - GenerateMultiGroupLevelB, GenerateMultiGroupLevelC, GenerateMultiGroupLevelD
   ...
   async function Generic2(Dir, Num, Name, GeomModel, VariabiliObj, Tractor, UniversalGeom) {
      ...
      function GenerateGroupLevelB(a, b) {
         ...
         //DATI DI MOLTIPLICAZIONE
         const GroupParam = MoltiplicationGroup(Node);
...
export async function MicEngine(Parameters, OggettiObj, GeometrieObj, MaterialiObj) {
   ...
   if (Par.Renderer.Enable == true) {
      if (Par.Renderer.AmbientMap.Enable == true) {
         if (Par.Renderer.AmbientMap.Type == "png") {
            ...
            // textureCube.encoding = THREE.SRGBColorSpace;
            // textureCube.generateMipmaps = false; //se non ti servono
            // textureCube.minFilter = THREE.LinearFilter;
   ...
   function updateMaterialsBlink(delta) {
      ...
      for (let i = 0; i < MaterialArray.length; i++) {
         ...
         // if (!m || !m.userData || m.userData.blink === undefined) continue;
         // if (m && m.userData && !m.userData.blink === undefined) {
         //    console.log("0");
         //    m.userData.blinkTime += delta;

         //    if (m.userData.blinkTime >= m.userData.blink) {
         //       console.log(m.userData.blinkState);
         //       m.userData.blinkTime = 0;
